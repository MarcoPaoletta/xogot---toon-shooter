...
for t in test_camera test_facing test_weapons test_bandit_bounds test_cover test_crates test_wheel test_scenes_runtime test_hazards; do
...
lines=[l.strip() for o in d.get('output',[]) for l in o.split(chr(10)) if l.strip().startswith(('PASS','FAIL','INFO'))]
